...
Incorporates patch described in Issue #37 on the google code page:
https://code.google.com/p/pyshp/issues/detail?id=37
"""
...
class Reader:
    ...
    def shapes_iter(self):
        """Returns iterator over entire file."""
        return self._shapes_iter(self, self.__getFileObj(self.shp),
                                 self.__shape)

    class _shapes_iter(object):
        """Replacement for shapes() method, can return shapes in a
           stream-like format."""
        def __init__(self, reader, fileobj, shapemethod):
            self.index = 0
            self.reader = reader
            self.shp = fileobj
            self.shapemethod = shapemethod
            # Skip over header
            self.shp.seek(100)
        ...
        def has_next(self):
            return self.shp.tell() < self.reader.shpLength

        def next(self):
            if self.has_next():
                return self.shapemethod()
            raise StopIteration
